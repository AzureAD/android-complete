...
.DESCRIPTION
    ...
    JSON schema (all fields optional except title + verdict):
    {
      ...
      // REQUIRED recommendation block for an ADO case — CASE-level, rendered ONCE at the END as "Proposed test steps".
      // NOT optional: you just walked the case step-by-step, so record how its steps should be reworded. If the case
      // reads fine, say so explicitly ("proposedScope": "No change needed", "proposedSteps": []) — an empty block is a
      // conclusion, an ABSENT block is an omission and renders as a visible "MISSING" warning.
...
if ($Command -eq 'summary') {
    ...
    foreach ($rf in $runFiles) {
        ...
        # Proposed test steps are MANDATORY per ADO case (empty-with-"No change needed" counts as done).
        # Rolling this up here is what catches a whole batch silently skipping it (it once went 0/26).
        if ($tcId -and -not ((Val $r 'proposedScope') -or (Val $r 'proposedSteps'))) {
    ...
    [void]$sm.AppendLine("")
    if ($missingProposed.Count) {
        $mp = ($missingProposed | Sort-Object -Unique) -join ', '
        [void]$sm.AppendLine("> **&#9888; Proposed test steps MISSING on $(@($missingProposed|Sort-Object -Unique).Count) case(s):** $mp — every ADO case report must carry ``proposedScope`` + ``proposedSteps`` (use ``""No change needed""`` + ``[]`` if the case reads fine).")
        [void]$sm.AppendLine("")
    }
    ...
    $mpHtml = ''
    if ($missingProposed.Count) {
        $mpU = @($missingProposed | Sort-Object -Unique)
        $mpHtml = "<p style='background:#fff4ce;border-left:4px solid #c19c00;padding:10px 14px;margin:14px 0'><b>&#9888; Proposed test steps MISSING on $($mpU.Count) case(s):</b> $(He ($mpU -join ', ')) &mdash; every ADO case report must carry <code>proposedScope</code> + <code>proposedSteps</code> (use <code>&quot;No change needed&quot;</code> + <code>[]</code> if the case reads fine as written).</p>"
    }
...
"@
    ...
    if ($missingProposed.Count) {
        $mpU = @($missingProposed | Sort-Object -Unique)
        Write-Warning "Proposed test steps MISSING on $($mpU.Count) case(s): $($mpU -join ', ') -- add proposedScope + proposedSteps (or 'No change needed' + []) to each run.json and re-render."
    }
    return
}
...
function ProposedMd($case) {
    ...
    if (-not ($scope -or $steps)) {
        # MANDATORY for an ADO case: the runner just walked the steps and is its best-placed reviewer.
        # An empty block ("No change needed" + []) is a conclusion; an ABSENT block is an omission -- say so loudly.
        $ado = Val $case 'ado'
        if ($ado -and (Val $ado 'testCaseId')) {
            return "## Proposed test steps`n`n> **MISSING** — every ADO test case report must include a ``proposedScope`` + ``proposedSteps`` block (state ``""No change needed""`` with an empty ``proposedSteps: []`` if the case reads fine as written). See references/test-reporting.md.`n`n"
        }
        return ''
    }
    $sb = New-Object System.Text.StringBuilder
...
function ProposedHtml($case) {
    ...
    if (-not ($scope -or $steps)) {
        $ado = Val $case 'ado'
        if ($ado -and (Val $ado 'testCaseId')) {
            return "<h2>Proposed test steps</h2><p class='bad'><b>&#9888; MISSING</b> &mdash; every ADO test case report must include a <code>proposedScope</code> + <code>proposedSteps</code> block (state <code>&quot;No change needed&quot;</code> with an empty <code>proposedSteps: []</code> if the case reads fine as written).</p>"
        }
        return ''
    }
