...
**As of AB#3683194 (Fadi feedback, Jun 2026) all header dates are auto-stamped by `bootstrap-report.ps1` from the resolved rolling 7-day window.** Authors no longer edit them. The previous Sun-Sat calendar-aligned model produced stale windows when the script was run mid-week (e.g. a Thursday run would emit a 4-day partial window and drop the last complete week — the exact "missed the last 6 days" the developer reported). The rolling model always covers the 7 fully-elapsed days immediately before the invocation.
...
| **`<div class="meta">` line 2: 60-day trend** | The Sun-Sat weekly-bucketed trend span. `sixtyDayEnd = startofweek(curEnd)` (Sunday that OPENS the current in-progress week, exclusive); `sixtyDayStart = sixtyDayEnd − 56d`. Displayed as the last COMPLETE Saturday (`sixtyDayEnd − 1`) for reader-friendliness. | Auto-stamped by bootstrap. |
| **`Generated <strong>…</strong>`** | The UTC date the report was produced. | Auto-stamped by bootstrap to `[datetime]::UtcNow.Date`. Do **not** hardcode it in any generator/assembler. If you rebuild the body programmatically, re-derive it from the clock with a **local-date** formatter (`new Date().toLocaleDateString('en-CA')` in Node, `(Get-Date).ToString('yyyy-MM-dd')` in PowerShell), never copy a literal from a prior file. ⚠️ Avoid `new Date().toISOString().slice(0,10)` — `toISOString()` is UTC and stamps tomorrow's date when you run in the evening in a UTC-negative timezone (a real off-by-one-day this skill hit). Bootstrap uses `[datetime]::UtcNow.Date` which sidesteps this. |
