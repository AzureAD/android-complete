...
|---|---|
| `<title>` and `<h1>` block | Reporting window dates (see **Date fields** below). The "Generated …" date is **auto-stamped to the run date by `bootstrap-report.ps1`** — you normally don't touch it. |
...
There are two *kinds* of date in the header, and conflating them is the v8 "Generated 2026-06-15 on a file built 2026-06-18" bug:

| Field | Meaning | How it's set |
|---|---|---|
| **Generated `<strong>…</strong>`** | The day you actually produced the report (system clock, **local** date). | **Auto-stamped by `bootstrap-report.ps1`** to today's date right after it copies the template. Do **not** hardcode it in any generator/assembler — if you rebuild the body programmatically, re-derive it from the clock with a **local-date** formatter (`new Date().toLocaleDateString('en-CA')` in Node, `(Get-Date).ToString('yyyy-MM-dd')` in PowerShell), never copy a literal from a prior file. ⚠️ Avoid `new Date().toISOString().slice(0,10)` — `toISOString()` is **UTC** and stamps tomorrow's date when you run in the evening in a UTC-negative timezone (a real off-by-one-day this skill hit). |
| **Reporting week** (`<title>`, `<h1>`, meta line) | The Sun→Sat bucket the data covers. | Author-set from the reporting Sunday: `start = <sunday>`, `end = <sunday> + 6d` (Saturday). This is the `startofweek` bucket label used throughout the queries. |
| **Baseline week** | The prior complete week the WoW deltas compare against. | `<sunday> − 7d` → `<sunday> − 1d`. |
| **60-day window** | The trend span feeding the sparklines / `bucket-trends.js`. | `<sunday> − 56d` (start, inclusive) → `<sunday> + 6d` (Saturday of the reporting week). That's **9 weekly buckets** inclusive of the reporting week — match the count you actually pulled, don't assume. |

Bootstrap deliberately leaves the reporting-week / baseline / 60-day dates **author-set** because the Sunday bucket must be verified against the user's intended week (off-by-one-week is the #1 silent error — see SKILL.md "Inputs to confirm"). Only the mechanical *Generated = today* stamp is automated.
