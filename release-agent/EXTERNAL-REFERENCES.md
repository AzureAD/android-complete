...
| ADO build def **397224** | Android Build Release (org msazure / project One) | readiness `build_access`, Phase-5 stage starters | az | Phase 5 locates the matching release-branch run (or future AndroidBuildBroker1ES resource-linked run) and starts `Release Sign Off`, `Upload What's New`, `Upload Alpha`, and owner-approved `100% Beta - Play Store`; Friday Beta starts additionally require manager approval; definition page: https://msazure.visualstudio.com/One/_build?definitionId=397224 |
| ADO build def **475778** | AndroidBuildBroker1ES Authenticator build | Phase-2 Authenticator RC evidence + Phase-5 final Auth build/version/link | az | Phase 5 reads the latest completed successful run on `state.versions.authenticator` (`release/YYYY/MM/DD`) and records id/version/commit into `state.pipeline_runs.final_auth`; definition page: https://msazure.visualstudio.com/One/_build?definitionId=475778 |
| ADO build def **355246** | AndroidBuild-1ES final Authenticator release-app build | Legacy reference only | az | No longer the Phase-5 source of Authenticator version/build evidence |
...
| **Teams channel: "General" (Android DevX ↔ partner release sync)** | team `be33b3e7-c501-4225-9413-3b88046f3eb3` · channel `19:715820c336d3454bbd75ef0bad68e460@thread.tacv2` | Phase-4 `release_announcement` target | MCP (**microsoft_teams** — channel ROOT posts; WorkIQ only does replies) | "Auth Client Android SDKs <Month Year> Release" post; redirect for tests via the `post_to` mock knob |
