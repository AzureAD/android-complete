...
| ADO build def **355246** | AndroidBuild-1ES final Authenticator release-app build | Phase-5 payload/tag + app-version/source selection | az | Successful build on `release/YYYY/MM/DD`; exact built commit plus numeric `N.N.N` build tag is the only app-version source, but the Phase-5 visible release-build link comes from recorded def 475778 state |
