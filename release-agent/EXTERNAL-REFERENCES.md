...
| **Teams chat: "Android Core Team"** | thread `19:976a859f167f44e59c4ceca8b1d23581@thread.v2` | Phase-0 `flight_reminder` target | MCP (WorkIQ) | LIVE target; dry-run posts to the owner's own chat |
...
## Outbound email recipients (LIVE runs only)

> In **dry-run**, every outbound email goes to the **release owner** instead of these.
> Real recipients are used only on a `--live` release.
