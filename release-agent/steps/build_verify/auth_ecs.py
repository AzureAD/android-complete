...
EVIDENCE = (PipelineScope(PipelineSlot.AUTH),)
ID = "auth_ecs"
KIND = "agent"
...
def verify_auth_ecs(context):
    ...
    ab = context.input("auth_build", MISSING)
    if ab is MISSING:
        auth_branch = (getattr(context.release, "versions", None) or {}).get("authenticator")
        if not auth_branch:
            return Blocked(f"{label}: no authenticator branch on state.versions yet — run "
                           f"orchestrator_health first.")
        ok, ab, detail = context.services.pipelines.find_auth_ecs_build(auth_branch)
        if not ok:
    ...
    if int(rc_num) < int(latest_rc(context).get("rc") or 0):
    ...
    invalidated = ()
    if latest_rc(context).get("auth"):
        runs = thaw(context.evidence.pipeline_runs)
        runs["rcs"][-1].pop("auth")
        invalidated = (PipelineEvidence(runs),)
    if status != "completed":
        return InProgress(
            f"{label} build {build_id} is still running (status: {status}) — Scout is polling "
            f"every 30 min and will capture its UI tests when it completes.", links=links, updates=invalidated)
    ...
    if result not in ("succeeded", "partiallySucceeded", "failed", "canceled"):
        return Blocked(f"{label}: unknown terminal build result: {result!r}.", links=links, updates=invalidated)
    if result in ("failed", "canceled"):
        evidence = stash_auth(context, rc_num, {
    ...
    tb = context.input("test_build", MISSING)
    if tb is MISSING:
        ok, tb, detail = context.services.pipelines.find_auth_ui_test_build(build_id)
        if not ok:
            hint = " — run `az login`" if str(detail).startswith("AUTH") else ""
            return Blocked(f"{label}: could not query the post-build UI tests ({detail}){hint}.",
                           links=links, updates=invalidated)
    if not tb:
    ...
    capture = context.input("capture", MISSING)
    test_status = context.input("test_status", MISSING)
    if test_status is MISSING:
        if suites is not MISSING or capture is not MISSING:
            ...
            ok, test_status, _, detail = context.services.pipelines.get_build_status(AUTH_ORG, AUTH_PROJECT, tb)
            if not ok or not test_status:
                return Blocked(f"{label}: could not read UI-test build status ({detail}).",
                               links=links, updates=invalidated)
    if test_status != "completed":
        return InProgress(f"{label} UI-test run {tb} is still running (status: {test_status}).",
                          links=links, updates=invalidated)
    if capture is MISSING:
        ok, capture, detail = context.services.pipelines.collect_auth_ui_evidence(tb, build_id, rc_num)
        if not ok:
    ...
    if issues:
        return Blocked("; ".join(issues) + "; retry verification when results are available.",
                       links=links)
    evidence = stash_auth(context, rc_num, snapshot)
    ...
    return Done(
        ...
        f"this with MRWP and decides go/hold.", links=links, updates=(evidence,))


def build(context: StepContext):
    return verify_auth_ecs(context)
