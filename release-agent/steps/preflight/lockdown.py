...
AAD-gated web app the engine can't read, so the flow is two hops:
  ...
  1. GATHER (skill): `build(context)` returns a NeedsSkill describing the browser
...
def release_window(context, cfg: dict | None = None):
...
def decide(context, raw_periods, cfg: dict | None = None):
    ...
    win_start, win_end = release_window(context, cfg)
...
def build(context: StepContext):
    ...
    return NeedsSkill(
        ...
        payload={
            ...
            "periods_json": None,
            "followup_command": (f"check-lockdown --release {context.release.release_id} "
