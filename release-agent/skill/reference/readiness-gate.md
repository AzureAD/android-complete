...
**Render the table ONCE, right before the attestations (step 3b) — not before.** Steps 1–3a are quiet setup (auto verifiers, silent-runs, scout checks); don't render a half-done table mid-way. The single, authoritative table render is in **step 3b**, where all auto items show ✅ and only attests are Outstanding. (Exception: if the user explicitly asks for status mid-gate, show the checklist per the behaviour-dispatch rule.)

1. **Compute the auto results quietly.** Run `verify --release <id>` (runs the Python auto verifiers — `build_access`, `mcp_servers` — and prints terse `[OK]/[FAIL]` lines, not the big table). Don't paste anything yet; you're just resolving the auto items. There are two item types: `[auto]` (Scout verifies) and `[attest]` (you confirm); all must be satisfied.

2. **Enable unattended (silent) runs.** The daily digest, Teams reminders, browser (CCOA/lockdown) checks, **and the readiness on-call/telemetry checks (ICM + Kusto MCP)** run without focus, so they must not stall on a prompt. This is the `silent_perms` item. Call **`m_get_settings`**, read `permissions.servers` **silently — never paste raw JSON**; surface only the plain-language choice. Satisfied when ALL `required_servers` (`shell`, `workiq`, `playwright`, `kusto`, `icm`) are **Allow** (`autoApprove: true`). **Never hard-blocks.**
    - **Already all on Allow** → `record-check --release <id> --item silent_perms --status pass --detail "shell/workiq/playwright/kusto/icm auto-approved"`. Continue to 3a **without saying anything** — no table, no prompt.
...
3a. **Run the scout-assisted `[auto]` checks yourself — silently and WITHOUT asking permission.** These are verified, not attested: **never ask the user "can I run the ADX query / on-call check?"** — just run them. The `kusto` and `icm` MCP servers are auto-approved (that's what `silent_perms` confirmed in step 2), so the calls don't prompt; asking the user is both unnecessary and contradicts silent-runs. Record each result with `record-check` (no per-check prose — that buries things). `silent_perms` was handled in step 2, so here it's just `oncall_now` + `adx_access`:
...
3b. **Now render the table — ONCE — and ask the attestations in the SAME message.** All auto items (`build_access`, `mcp_servers`, `silent_perms`, `oncall_now`, `adx_access`) are now resolved. Run `checklist --release <id>` and **paste its table verbatim into the same assistant message as your attestation question** — auto items show ✅, only the four attests are Outstanding. Running the command is not enough; the user only sees what you paste, and **this is the first time the table appears in the conversation**. **Do NOT send an attestation question in a message that lacks the table.** Then `m_ask_user` for the four attestations (`play_console_access`; `oncall_window` — CCD‑7→CCD+14 dates in the table; `saw_ame`; `yubikey`). Offer: **"All confirmed"**, **"I'm scheduled on-call during the window"**, **"I can't open Play Console"**, **"I don't have a SAW machine"**, **"I don't have a YubiKey"**. Never replace the table with a prose list.
