...
- **`auto`** — **Scout resolves it** (verifies programmatically, pass/fail). Two execution sources, both shown as `[auto]`:
  - *Python-verified* (default): `build_access` (both ADO build definitions, via `az`) and `mcp_servers` (ICM + Kusto/ADX + Teams + Mail MCP servers registered in Scout). The engine's `verify`/`sign` runs these.
  - *Scout-assisted* (`source: scout`): the engine can't reach the MCP/Scout-settings, so **YOU run the check** and record the result (step 3). Fail-closed **except the opt-out items `silent_perms` and `teams_notify`**. Today: `mail_fallback_live` (the exact Mail tool is loaded now), `oncall_now` (ICM current on-call), `adx_access` (Kusto `print 1`), `ccd_confirmed` (CCD reconciled with pipeline 3038's override), `silent_perms` (Scout permissions allow unattended runs), `teams_notify` (Scout Teams bot reachable for the digest — degrades to email-only).
...
2. **Silent: verify the Mail fallback is live, then enable unattended runs.**
   - **`mail_fallback_live` is hard/fail-closed:** inspect the current session's available tools and require the exact `microsoft_mail-SendEmailWithAttachments` tool named by `required_tools`. A config entry, `servers.mail.enabled:true`, or `autoApprove:true` is NOT proof the server was loaded. If the tool is absent, record `record-check --release <id> --item mail_fallback_live --status fail --detail "Mail MCP is configured but microsoft_mail-SendEmailWithAttachments is not loaded — restart Scout"` and STOP the entry gate. Do not offer an opt-out and do not sign readiness. After the user restarts Scout, reload the skill, verify the exact tool is present, and record `pass`.
   - **Then `silent_perms`:** the daily digest, Teams reminders, browser (CCOA/lockdown) checks, release-email fallback, and the on-call/telemetry (ICM + Kusto MCP) checks run without focus, so they must not stall on a prompt. Call **`m_get_settings`**, read `permissions.servers` **silently — never paste raw JSON**. Satisfied when ALL `required_servers` (`shell`, `workiq`, `playwright`, `kusto`, `icm`, `mail`) are **Allow** (`autoApprove: true`). Permission opt-out may degrade; missing live Mail capability may not.
    - **Already all on Allow** → `record-check --release <id> --item silent_perms --status pass --detail "shell/workiq/playwright/kusto/icm/mail auto-approved"`. Continue silently.
    - **One or more NOT on Allow** → offer via `m_ask_user`: **"Enable silent runs (recommended)"** vs **"Proceed without — I'll get occasional prompts"**. Explain the downside: those background checks pop a prompt when Scout isn't focused and can stall until you open Scout.
        - **Enable** → if `permissions.allowModelPermissionsChange` is `false`, ask them to turn on **Settings → Permissions → "When blocked, let Scout ask instead of stopping"** (read-only from the model — only they can flip it). Once `true` (often already on), call **`m_request_permission_escalation`** with `servers: { workiq:{autoApprove:true}, playwright:{autoApprove:true}, kusto:{autoApprove:true}, icm:{autoApprove:true}, mail:{autoApprove:true} }` (add `shell` if off); they click **Allow** once → re-read `m_get_settings` → `record-check … --status pass --detail "enabled silent runs"`.
   ...
   **Canonical silent-runs wording** (near-verbatim; no jargon, never show raw JSON):
   - **Master switch ON, WorkIQ/browser not on Allow** →
     > *Before we go further, I'd like to enable **silent runs** so the release keeps working when Scout isn't in focus. The daily status digest, Teams reminders, release-email fallback, browser (CCOA/lockdown) checks, and on-call/telemetry checks all run on a background schedule — if they aren't auto-approved, they hit a permission prompt and stall until you open Scout. I can set that up now: I'll request auto-approval for **WorkIQ**, the **Mail fallback**, the **browser**, and the **ICM + Kusto** servers, and you just click **Allow** once. Want me to go ahead?*
   ...
   - **Master switch OFF** (`allowModelPermissionsChange:false`) →
     > *To enable silent runs I need to request auto-approval for WorkIQ, the Mail fallback, and the browser — but first Scout needs one switch turned on that only you can flip: **Settings → Permissions → "When blocked, let Scout ask instead of stopping."** Once that's on, tell me and I'll request the approvals (you'll click **Allow** once). Prefer not to? I'll record this as running without silent approvals — you'll just get an occasional prompt when Scout isn't focused.*
...
3. **Silent: run the scout-assisted `[auto]` checks and record each** — don't ask the user; they're verified, not attested. **Never ask permission to run the ADX/on-call MCP calls — they're auto-approved.** No table, no per-check prose:
    ...
    - **`ccd_confirmed` (CCD validity — temporal + pipeline):** run **`check-ccd --release <id> --json`** → `{ccd, override, ccd_conflict, status, days_to_ccd, runway_days, compressed, phase0_open}`. Act on `status` (checked in priority order):
        - **`past`** (CCD is before today — `days_to_ccd` < 0; e.g. a current-month release whose 2nd-Wednesday default already passed) → **BLOCK; must reschedule to a future date.** `--default` won't help (the default IS the past date). If `override` is present AND itself in the future, offer to adopt it; otherwise ask the user for a real future CCD (`m_ask_user`: "CCD `<ccd>` is `<days_to_ccd humanized>` — pick a future Code Complete Date."). Apply with **`set-ccd --release <id> --date <future> --confirm --reason "CCD was in the past"`**, **re-run `check-ccd`** until it's no longer `past`, then continue to whatever status it now reports. Do NOT record pass while `past`.
        - **`conflict`** (pipeline override is an in-month date that DIFFERS) → **surface both to the user** and ask which is right (`m_ask_user`: "Pipeline says `<override>`, this release is anchored to `<ccd>` — which is the real Code Complete Date?"). Then apply their choice with **`set-ccd --release <id> … --confirm --reason "<why>"`** (`--date <override>` to adopt the pipeline, or `--default` to keep the 2nd-Wednesday default), **re-run `check-ccd`** until `match`, then `record-check … --item ccd_confirmed --status pass --detail "reconciled to <ccd>"`. Do NOT record pass while it still conflicts.
        - **`unreadable`** (couldn't read pipeline 3038) → **attest fallback**: ask the user to confirm the CCD by hand ("I couldn't read the pipeline; is `<ccd>` the correct Code Complete Date?"). On yes → `record-check … --item ccd_confirmed --status pass --detail "pipeline unreadable — CCD <ccd> confirmed manually"`. On a different date → `set-ccd … --date <their date> --confirm --reason` first, then record pass.
