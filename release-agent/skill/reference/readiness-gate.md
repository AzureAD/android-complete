...
3. **Silent: run the scout-assisted `[auto]` checks and record each** — don't ask the user; they're verified, not attested. **Never ask permission to run the ADX/on-call MCP calls — they're auto-approved.** No table, no per-check prose:
    ...
    - **`ccd_confirmed` (pipeline reconciliation):** run **`check-ccd --release <id> --json`** → `{ccd, override, ccd_conflict, status}`. Act on `status`:
        - **`match`** (no pipeline override, or the override equals the CCD) → `record-check --release <id> --item ccd_confirmed --status pass --detail "CCD <ccd> reconciled with pipeline"`.
