...
**Confirm the release NAME (ship month) at init.** A release is named for the month it **ships**, which is the code-complete month **+ 1** — a release whose CCD is in August is the **"September" release**. `init` stores this default (`target_month`, e.g. `2026-09`) and prints it ("Release name: September 2026 release"). Right after `init`, confirm it with the user via `m_ask_user` — e.g. *"This release code-completes Aug 13, so it's the **September 2026** release — right?"* — with chips **"Yes, September 2026"** / **"No, pick a different month"**. If they pick a different month, apply it with `set-target-month --release <id> --month <YYYY-MM>` (display-only — it does NOT touch `release_id`, the CCD, branches, or scheduling). This name is what every doc/comms step uses (bug-bash invite, announcements, the payload wiki page), so getting it right here prevents a project-wide misname.
