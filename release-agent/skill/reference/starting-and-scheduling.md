...
2. If missing, `m_create_automation`:
   ...
   - **schedule:** `cron: 0 8-18 * * *` — **hourly on the hour, 08:00–18:00 local, every day.** The first tick of the day is **08:00**, so a phase opening (e.g. Phase 2 on CCD+1) reaches the owner as an **8 AM "resume"**; the remaining daytime ticks are same-day catch-ups (idempotent + once-per-day de-dup ⇒ still one digest/day) and nothing fires overnight. (Cron is interpreted in **local** time, matching the CCD comms' `fire_at_local`.)
   - **teamsNotify:** `never`
   - **prompt:** From `C:\repos\android-complete\release-agent`, advance the active release **AUTONOMOUSLY** (no user is watching) and send the daily digest:
     ...
     2. **Run Scout's own steps until none remain.** Loop: run `next --json`; read `scout_pending`; if empty, go to step 3; else for EACH id: run `step-action --release <id> --phase <phase> --step <stepId>` (Phase 0 = `preflight`), perform the returned `needs_skill` `tool`+`payload` via the matching MCP tool (`workiq_send_email` / `workiq_send_chat_message` / `azure_devops-pipelines_run_pipeline`, honoring `test_redirect`), then finalize with `record-step … --status pass` — **UNLESS** the action names a follow-up engine command (e.g. `check-lockdown`, `record-localization-run`/`check-localization`), which you run instead as the action describes. **Headless safety copy:** whenever a `needs_skill` action has **`outbound: true`**, after performing it also `m_send_teams_message` a **ONE-LINE** courtesy copy — `🤖 [release <id>] Autonomous: <summary>.` — so the owner sees what went out (never paste the full email/message body). A scout step that records `attention` is left blocked (it surfaces in the digest). Do it silently.
...
3. **Register it** so it's torn down at close: `automation register --id <id> --name "Release push reminders" --release <YYYY-MM> --purpose "workday-hourly (08:00–18:00 local) autonomous advance (runs scout steps) + phase digest to owner (email + Teams); 8am first-of-day = phase-open resume"` — no `--step`, so it's recorded as a **release-level** automation (it advances the whole release, owns no step).

Do it silently as part of start (the user already opted into push). **Why workday-hourly, pinned to 08:00:** the phase-open "resume" must land in the morning, not overnight — so the first tick of every day is **08:00 local**. But a single 8 AM trigger would be *skipped* if the machine was off then; hourly through 18:00 gives same-day catch-ups (`tick` is idempotent — advancing no-ops once holding, and the digest de-dupes to one/day), so a missed 8 AM is picked up later that day while nothing wakes the owner at 1 AM.
