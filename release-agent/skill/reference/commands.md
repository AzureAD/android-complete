...
| Finalize / rollout auto-writers | `create-integration-prs`, `create-oneauth-common-pr`, `create-payload-wiki`, and `start-release-signoff` are scheduled release automation, not owner approval gates. Their workers run with `--execute --auto-approve --executor integration-pr-automation|oneauth-common-automation|payload-wiki-automation|release-signoff-automation`. If a provider result is uncertain, stop and recover the owned execution; do not retry. |
...
`--execute --auto-approve --executor release-signoff-automation` respectively.
