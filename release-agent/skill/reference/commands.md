...
| **Phase 2 — one RC poll** | `python -m orchestrator.cli poll-rc --release <YYYY-MM>` — `waiting` / `nudge` / `resolved` / `blocked` / `idle`. The on-demand poller's central cleanup rule removes it after resolved or blocked. |
