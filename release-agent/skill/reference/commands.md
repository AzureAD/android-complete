...
| **Simulate a mid-release point** (testing) | `python -m orchestrator.cli sim list` · `python -m orchestrator.cli sim run --scenario <name> [--freeze] [--json]` → fast-forwards the real engine to a scenario's target (`config/scenarios/<name>.yaml`), signs the entry gate + completes earlier phases offline, then stops `open`/`gate`/`done` at the target. `data: live` reads real `az`; `data: mock` uses the scenario's input mocks. Writes to `.sim-runs` (not real releases); `--freeze` snapshots state to `tests/fixtures/<name>.json` |
