...
- **User wants to test/validate a phase mid-release without running one from scratch** ("test phase 2", "simulate phase 2", "let me test the RC phase", "drop me in at the RC gate", "test the bug-bash phase"): this is the **sim** — run it for them via the shell, don't hand them python to type. Pick the scenario by intent (`sim list` shows all):
  - "test phase 2" / "test phase 2 against the real pipelines" / "does phase 2 work" → **`sim run --scenario build_verify_live`** (fast-forwards Phases 0-1 offline, runs the 4 verification steps against the **real** 2026-08 `az` runs, halts at the go_test gate).
  - "test phase 2 offline / quickly / without the network" → **`sim run --scenario at_rc_gate`** (same flow, fully mocked).
  - "drop me at phase 2 so I can step through it myself" → **`sim run --scenario mid_build_verify_open`** (positions at entry, runs nothing).

  Run the command, then **paste the status output back to the user** (the phase map + step verdicts + `Pipelines:` line) and offer the follow-ups: `sim status` to re-view, `rc-report` for the test breakdown, or `approve` to clear the gate and roll into the next phase. The sim fast-forwards the **real engine** to the target — signs the entry gate + completes earlier phases from mocks, stops at `open`/`gate`/`done`. **It is a SANDBOX in `.sim-runs` — it NEVER touches the real release, so a normal `status` (real runs-root) will not reflect a sim run; use `sim status` to view the sandbox.** Follow-up commands need the sim runs-root: `--runs-root <printed path> <cmd>`. Scenarios live in `config/scenarios/*.yaml`; add one per phase as phases grow.
