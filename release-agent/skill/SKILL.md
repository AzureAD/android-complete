...
- **Distribution ownership:** separately preview balanced manual work and owner triage (Blocked-tagged cases plus applied automated failures). Never silently leave excluded cases with stale owners. The owner is excluded from manual work, not triage. Distribution apply aligns both shared case assignees and selected plan testers; preserve outcomes/tags and require both assignment surfaces to succeed.
