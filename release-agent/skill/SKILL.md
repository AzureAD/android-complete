...
- **Distribution source of truth:** ADO owns assignments and plan testers; never store/replay an internal allocation map. `distribute-tests --json` reads live ADO and shows only needed corrections, separately for manual work and owner triage. Only after explicit approval, pass that output's `--review-hash` with `--apply`; the command rereads ADO, rejects changed review inputs, corrects mismatches and reads back. Valid ADO needs no redistribution. Partial failures require reviewing live remaining corrections, not rollback. Keep only availability decisions and workflow status internally; progress reads live ADO.
...
- **Bug Bash progress counts:** count human manual/triage work, not all suite cases with the same assignee. Use the completed fill's automated-case classification to exclude automation-only Auth cases from both done and total, retain applied automation failures as triage, and preserve real manual completions. Owners/outcomes still come from live ADO; no assignment cache.
