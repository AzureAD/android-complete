...
- **User wants to test/validate a phase mid-release without running one from scratch** ("simulate a release at phase 2", "drop me in at the RC gate", "test the bug-bash phase"): run **`sim run --scenario <name>`** (list options with `sim list`). It fast-forwards the **real engine** to a scenario's target point — signs the entry gate + completes earlier phases from offline mocks, then stops at the target (`open` = positioned at entry, `gate` = holding at its gate, `done` = phase complete). A scenario with `data: live` validates the target phase against real `az` runs; `data: mock` is fully offline. State lands in `.sim-runs` (never a real release) — inspect it with `--runs-root <printed path> status`. Add `--freeze` to snapshot the produced state to `tests/fixtures/`. Scenarios live in `config/scenarios/*.yaml`; add one per phase as phases grow.
